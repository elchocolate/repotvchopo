...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Actualmente no hay Ningun Chikirywizard instalado desde %s.\n\nSeleciona 'Wizard Menu' para instalar Chikirywizard o 'Ignorar' para no volver a ver este mensaje.\n\nGracias por escoger %s." % (ADDONTITLE, ADDONTITLE)
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.new = kwargs['new']
			self.icon = kwargs['new']
			self.fanart = kwargs['fanart']
			self.msgupdate  = "NUEVA ACTUALIZACION DISPONIBLE CHIKIRYWIZARD:\n[COLOR %s]%s[/COLOR]\n\nActual Version: v[COLOR %s]%s[/COLOR]\nUltima version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*INSTRUCCIONES*      Para una Instalacion Limpia Pulse En Fresh Install, *Para Solo Actualizar Sin Borrar Datos Pulse Sobre Normal,      *Para Posponer La Actualizacion En 3 dias Pulse Sobre Update.[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
			self.msgcurrent = "Su Wizard Esta Actualizado:\n[COLOR %s]%s[/COLOR]\n\nactual Version: v[COLOR %s]%s[/COLOR]\nUltima Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendado: Fresh install[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
