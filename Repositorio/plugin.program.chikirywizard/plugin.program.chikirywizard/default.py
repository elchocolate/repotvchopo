	...
	if type == 'gui':
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el cierre[/COLOR] de kodi para [COLOR %s]este perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]NO[/COLOR][/B]", nolabel="[B][COLOR green]CERRAR[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
...
def thirdPartyInstall(name, url):
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el cierre[/COLOR] de kodi para [COLOR %s]este perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]NO[/COLOR][/B]", nolabel="[B][COLOR green]FORZAR[/COLOR][/B]")
	if todo == 1: wiz.reloadFix()
	else: wiz.killxbmc(True)
