...
def maintMenu(view=None):
	...
	addDir ('[I]<< Volver al Menu Principal WIZARD-CHOPO[/I]', icon=ICONMAINT, themeit=THEME2)	    
...
def removeAddonMenu():
    ...
    if len(selected) > 0:
        ...
        else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el cierre de[/COLOR] Kodi o [COLOR %s]Recargar el perfil[/COLOR]?La Opcion recomendada es Cerrar.[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR spinggreen]Cerrar Kodi[/COLOR][/B]")
        ...
        if yes_pressed:
            ...
            if int(float(percent)) > 0:
                ...
                if int(float(errors)) > 0:
                    yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')), 'Completado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Si[/COLOR][/B]')
                    if yes:
