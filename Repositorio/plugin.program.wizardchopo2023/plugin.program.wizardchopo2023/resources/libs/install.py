...
def wipe():
    # --- BACKUP secure.dat ---
    secure_src = xbmcvfs.translatePath('special://userdata/secure.dat')
    secure_dst = xbmcvfs.translatePath('special://temp/secure.dat')

    if os.path.exists(secure_src):
        try:
            shutil.copy2(secure_src, secure_dst)
        except:
            pass
    ...
    # Añadir repos, super y whitelist como antes
    ...
    if CONFIG.KEEPWHITELIST == 'true':
        ...
        whitelist_items = whitelist.whitelist('read')
        if len(whitelist_items) > 0:
    ...
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        ...
        for name in files:
            ...
            if name == 'secure.dat' and fold[-1] == 'userdata':
                logging.log("Keep secure.dat: {0}".format(os.path.join(root, name)))
            elif name == 'sources.xml' and fold[-1] == 'userdata' and CONFIG.KEEPSOURCES == 'true':
    ...
    # El resto del wipe sigue igual (carpetas, progress_dialog, clear_setting)
